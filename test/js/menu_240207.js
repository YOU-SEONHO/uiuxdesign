$(document).ready(function(){
    ...
    /* 메뉴에 마우스를 오버하면 - .header .gnb
       header에서 클래스 menu_over 추가 
       메뉴에 마우스를 아웃하면 - header에서 bg를 깔아줬기 때문에 header에 마우스 아웃
       header에서 클래스 menu_over 삭제
       
       mouseenter를 주면 click해도 작동됨 
       모바일에서 pc버전용으로 만든 코드가 작동됨
       pc인지 모바일인지 jquery에서 구분해서 pc에서만 작동되도록 제한
       css에서도 브라우저 사이즈로만 모바일/pc 구분
       jquery도 브라우저 사이즈로만 모바일 pc 구분 */
    ...
    $(window).resize(function(){ // 브라우저가 리사이즈될때마다 한번씩 실행
        device_chk()
    }) // $(window).resize
    ...
    /* 마지막 1차메뉴의 마지막 2차메뉴에서 포커스 아웃되면 메뉴 오버를 해제
       제일 좋은 방식은 메뉴 옆에 언어/찾기라던가 다른링크가 있고 그 링크에 포커스가 됐을때
       out되는것이 제일 좋음 */
    ...
    /* 1차메뉴를 클릭하면 감싸고 있는 li태그에 sub_open이라는 클래스를 추가
       ...
       하나의 메뉴를 클릭해서 열고 다른 메뉴를 클릭하면 이전 메뉴도 닫힘 (지금 클릭한것만 열림)
